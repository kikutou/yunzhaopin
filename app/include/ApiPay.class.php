...
class ApiPay extends common{ 
	function payAll($dingdan,$total_fee,$paytype=''){ 
		...
		if($order['order_state']!='2' && $order['id']){
			...
			$emaildata['moblie']=$config['sy_webmoblie'];
			...
			if($order['type']=='1' && $order['rating'] && $member['usertype']!='1'){
				...
				if($nid){ 
					$this->obj->DB_insert_once("company_pay","`order_id`='".$order['order_id']."',`order_price`='".$order['integral']."',`pay_time`='".time()."',`pay_state`='2',`com_id`='".$order["uid"]."',`pay_remark`='"."购买".$config['integral_pricename']."',`type`='1',`pay_type`='2',`did`='".$config['did']."'");
					$sendMail = 1;
				}
