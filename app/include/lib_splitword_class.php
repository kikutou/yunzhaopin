...
class SplitWord
{
  	...
  	for($i=($spLen-1);$i>=0;$i--){
  		...
  		else
  		{
  		  ...
  		  else 
  		  {
  		  	...
  		  	{
  		  		if(mb_ereg($this->EspecialChar."$",$spwords[$i],$regs)){
  		  				$spwords[$i] = mb_ereg_replace($regs[0]."$","",$spwords[$i]).$spc.$regs[0];
  		  		}
  		  		if(!mb_ereg("^".$this->CommonUnit,$spwords[$i]) || $i==0){
  		  			$this->ResultString = $spwords[$i].$spc.$this->ResultString;
  		  		}else{
  ...
  function ParOther($WordArray)
  {
  	...
  	for($i=$wlen;$i>=0;$i--)
  	{
  		if(mb_ereg($this->CnSgNum,$WordArray[$i])){
  			$rsStr .= $spc.$WordArray[$i];
  			if($i>0 && mb_ereg("^".$this->CommonUnit,$WordArray[$i-1]))
  			{ $rsStr .= $WordArray[$i-1]; $i--; }
  			else{
  				while($i>0 && mb_ereg($this->CnSgNum,$WordArray[$i-1]))
  				{ $rsStr .= $WordArray[$i-1]; $i--; }
  			}
  			continue;
  		}
    ...
    for($i=0;$i<$slen;$i++){
      if(ord($str[$i]) < 0x81)
      {
        if(ord($str[$i]) < 33){
          ...
          $prechar=0;
          continue;
        }else if(mb_ereg("[^0-9a-zA-Z@\.%#:/\\&_-]",$str[$i]))
        {
          if($prechar==0)
        ...
        }else
        {
        	...
        	{
        	  if(mb_ereg("@#%:",$str[$i])){ $okstr .= $str[$i]; $prechar=3; }
